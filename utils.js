...
const getCalendar = (props) => {
    ...
    let dayOfWeek = new Date(`2021-${month + 1}-2`).getDay()
    ...
    const btns = [{text: '<-', callback_data: `${SET_DATE}__prev_${prevYear}.${month-1}`}, {text: '->', callback_data: `${SET_DATE}__next_${nextYear}.${month+1}`}]

    const calendar = calendarData.map(row => {
        return  row.map(item => {
            return ({ text: item, callback_data: item !== '-' ? `${SET_DATE}__${item}.${month + 1}.${current.getFullYear()}` : item })
        })
    })
    calendar.push(btns)
    return calendar
